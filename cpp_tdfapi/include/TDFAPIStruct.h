...
struct TDF_CODE
{
    ...
    int nType;                            
	char szWindType[8];		//windType : FUR(期货),测试
	int nRecord;			//当日编号(marketID*64*1024+record)，单活有效
...
enum TDF_ENVIRON_SETTING
{
	...
	TDF_ENVIRON_HEART_BEAT_FLAG,			//心跳数据发送方式(0: 取消发送心跳包 1：没有数据发送的时候发送心跳数据，2：有规律发送心跳数据s)
	...
	TDF_ENVIRON_SWITCH_SAME,				//switch双活模式下有小，保证一个交易日的前收和开盘只有一个有效值
};
//双活模式设置
